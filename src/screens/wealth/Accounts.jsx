  ...
  return (
    <div>
      ...
      {visible.length === 0 ? (
        ...
        <div className="mn-list" style={{ marginTop: 22 }}>
          {visible.map((a) =>
            a.type === 'credit_card' ? (
              <CreditCardRow key={a.id} account={a} transactions={transactions} members={members} onEdit={setEditing} />
            ) : (
...
function CreditCardRow({ account, transactions, members, onEdit }) {
  ...
  return (
    <button type="button" className="mn-row bud-row" onClick={() => onEdit(account)}>
        ...
        {util !== null ? (
          <>
            ...
            <div className="ov-muted" style={{ marginTop: 4 }}>
              Owed <span className="fig">{formatMoney(account.balance)}</span> of{' '}
              <span className="fig">{formatMoney(account.credit_limit)}</span> ({formatPct(util)})
              {est && (
                <>
                  {' · Est. next statement '}
                  <span className="fig">{formatMoney(est.amount)}</span>
                  {' since '}
                  {cycle.lastClose.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}
                </>
              )}
              {account.due_day && ` · due ${account.due_day}th`}
            </div>
          </>
        ) : (
          <div className="ov-muted">
            Owed <span className="fig">{formatMoney(account.balance)}</span> · no limit set
          </div>
        )}
      </div>
    </button>
